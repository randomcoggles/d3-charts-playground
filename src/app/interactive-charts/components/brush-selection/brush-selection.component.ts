...
export class BrushSelectionComponent
  ...
  ngAfterViewInit() {
    ...
    this.http
      .get("assets/bubble-data02.json", { headers: { cacheable: "true" } })
      .pipe(delay(1000))
      .pipe(
        finalize(() => {
          this.dataReady = true;
  ...
  private getSelectionRanges(selection) {
    ...
    return {
      rangeY: { min: yMin, max: yMax },
      rangeX: { min: xMin, max: xMax },
      numberOfItems: newSelection.length,
    };
  }
